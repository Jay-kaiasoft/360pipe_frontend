...
const Todo = ({ setAlert, setSyncingPushStatus, syncingPullStatus }) => {
    ...
    // Debounce API calls when user toggles multiple checkboxes quickly
    const saveTodayTimeoutRef = useRef(null);
        ...
        if (res.status === 200) {
            let formattedTodos = res?.result?.map((todo, index) => ({
                ...
                dueDate: todo.dueDate
                    ? dayjs(todo.dueDate, "MM/DD/YYYY, hh:mm:ss A", true)
                    : null,
            })) || [];
            ...
            // Build checked list in one go (avoids duplicates and extra renders)
            const checked = formattedTodos
                ?.filter(t => t.isToday)
                ?.map(t => ({ id: t.id, customId: t.customId })) || [];

            setCheckTodoIds(checked);

            setTodos(formattedTodos);
        }
    };

    const saveTodaySelection = async ({ todoIds, removeIds, prevState }) => {
        const data = {
            source: "",
            isToday: false,
            todoIds: todoIds,
            removeTodoIds: removeIds,
        };

        // Keep your existing server contract (source/isToday depends on active tab)
        if (activeFilterTab === 0) {
            data.source = "All";
            data.isToday = false;
        } else if (activeFilterTab === 1) {
        ...
        if (res.status === 200) {
            ...
            if (prevState) {
                setCheckTodoIds(prevState.checkTodoIds);
                setRemoveTodoIds(prevState.removeTodoIds);
            }
            setAlert({
                open: true,
                message: res?.message || "Failed to update todo",
    ...
    // Save button removed, so no need to track "dirty" state
    ...
    const columns = [
                    ...
                    <div className='h-full flex justify-center items-center'>
                        <Checkbox
                            ...
                            onChange={(e) => {
                                ...
                                // Keep previous state for rollback if API fails
                                const prevState = {
                                    checkTodoIds,
                                    removeTodoIds,
                                };

                                // Compute next states synchronously
                                let nextChecked = [...checkTodoIds];
                                let nextRemoved = [...removeTodoIds];

                                if (newValue) {
                                    if (!nextChecked.some(item => item.id === currentId)) {
                                        nextChecked.push({ id: currentId, customId: currentCustomId });
                                    }
                                    nextRemoved = nextRemoved.filter(item => item.id !== currentId);
                                } else {
                                    nextChecked = nextChecked.filter(item => item.id !== currentId);
                                    if (!nextRemoved.some(item => item.id === currentId)) {
                                        nextRemoved.push({ id: currentId, customId: currentCustomId });
                                    }
                                }

                                // Optimistic UI update
                                setCheckTodoIds(nextChecked);
                                setRemoveTodoIds(nextRemoved);

                                // Debounced save
                                if (saveTodayTimeoutRef.current) {
                                    clearTimeout(saveTodayTimeoutRef.current);
                                }
                                saveTodayTimeoutRef.current = setTimeout(() => {
                                    saveTodaySelection({
                                        todoIds: nextChecked,
                                        removeIds: nextRemoved,
                                        prevState,
                                    });
                                }, 400);
                            }}
        ...
        },
        {
            ...
            // ✅ IMPORTANT: custom table needs renderCell to avoid rendering object
            renderCell: ({ value }) => (value ? dayjs(value).format("MM/DD/YYYY") : "-"),

            // ✅ DataGrid sorting (works when allowSorting=false)
            type: "dateTime",
            ...
            <div className='border rounded-lg bg-white w-full lg:w-full '>
                <DataTable
                    getRowClassName={(params) => {
                        ...
                        const dueDate = params.row.dueDate ? params.row.dueDate.toDate() : null;
                        if (!dueDate) return '';
