...
const SubuserTypeList = ({ setAlert }) => {
    ...
    const handleOpenDeleteDialog = (id) => {
        ...
        setDialog({ open: true, title: 'Delete Sub-User Type', message: 'Are you sure! Do you want to delete this sub-user type?', actionButtonText: 'yes' });
    }
    ...
    const handleDeleteSubUserType = async () => {
        ...
        if (res.status === 200) {
            ...
            setAlert({
                open: true,
                message: res?.message || "Failed to delete sub-user type",
                type: "error"
            });
        }
    }
    ...
    const actionButtons = () => {
        return (
            ...
            //   component={
            <div>
                <Button type={`button`} text={'Add Sub-User Type'} onClick={() => navigate("/dashboard/manageusers/add")} startIcon={<CustomIcons iconName="fa-solid fa-plus" css="h-5 w-5" />} />
            </div>
            //   }
            // />
        )
...
export default connect(null, mapDispatchToProps)(SubuserTypeList)
